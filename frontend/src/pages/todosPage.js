export const todosPage = () => {
  ...
  btnAdd.addEventListener("click", () => {
    ...
    // Realizar solicitud POST para agregar nueva tarea
    fetch("http://localhost:4000/todos", {
      ...
      .then(data => {
        ...
        location.reload(); // Recargar la página para ver la tarea actualizada
      });
  });
  ...
  fetch("http://localhost:4000/todos", {
    ...
    .then((data) => {
      data.todos.forEach((todo) => {
        ...
        btnEdit.classList.add("bg-yellow-500", "text-white", "p-1", "mr-2", "rounded");
        btnEdit.textContent = "Editar";
        btnEdit.addEventListener("click", () => {
          const newTitle = prompt("Nuevo título:", todo.title);
          const newCompleted = confirm("¿Está completada?");

          // Solicitud PUT para actualizar la tarea
        ...
        btnDelete.classList.add("bg-red-500", "text-white", "p-1", "rounded");
        btnDelete.textContent = "Eliminar";
        btnDelete.addEventListener("click", () => {
          if (confirm("¿Seguro que deseas eliminar esta tarea?")) {
            // Solicitud DELETE para eliminar la tarea
            fetch(`http://localhost:4000/todos/${todo.id}`, {
              method: "DELETE",
              credentials: "include",
            })
